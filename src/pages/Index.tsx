...
export default function Index() {
  ...
  return (
    <main className="relative min-h-screen bg-black text-neon scanlines selection:bg-[color:var(--neon)] selection:text-black overflow-x-hidden">
      
      {/* CAMADA 1: Malha Estática (Fica no fundo de tudo) */}
      <div className="bg-binary-texture" aria-hidden="true" />

      {/* CAMADA 2: Fluxo Binário Dinâmico (ACELERADO) */}
      <div className="ghost-binary-layer flex justify-around">
        {[...Array(12)].map((_, i) => (
          <div 
            key={i} 
            className="ghost-column"
            style={{ 
              /* Reduzi de 30s/10s para 15s/5s para dobrar a velocidade média */
              animationDuration: `${15 + i * 5}s`, 
              opacity: 0.3 + (i % 4) * 0.1
            }}
          >
            {"010110011010101100110101101010".repeat(15)}
          </div>
        ))}
      </div>

      {/* CONTEÚDO PRINCIPAL - Elevado para ficar por cima do fundo */}
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        <section className="px-4 sm:px-8 py-16 sm:py-24 max-w-5xl mx-auto">
          <p className="text-terminal text-xs sm:text-sm mb-6">
          ...
          <div className="mt-10 grid grid-cols-2 sm:grid-cols-4 gap-2 text-[10px] sm:text-xs">
            {["KERNEL: OK", "Inteligência Artificial: sem regulação", "Famílias: definição desatualizada", "Contratos digitais: sem marco legal"].map((s, i) => (
              <div
                key={s}
                className={`border px-2 py-1 backdrop-blur-sm bg-black/60 ${
                  i === 0
        ...
        {/* PROBLEMA */}
        <section className="border-t border-[color:var(--neon)]/30 bg-black/40 backdrop-blur-sm">
          <div className="px-4 sm:px-8 py-16 max-w-5xl mx-auto">
            ...
            <div className="border border-[color:var(--neon)]/60 p-4 sm:p-6 space-y-4 bg-black/80 backdrop-blur-md">
            ...
            <div className="mt-6 border border-[color:var(--bug)] bg-[color:var(--bug)]/10 px-4 py-3 text-[color:var(--bug)] bug-glow text-xs sm:text-sm font-bold backdrop-blur-sm">
        ...
        {/* TESE */}
        <section className="border-t border-[color:var(--neon)]/30 bg-transparent">
          <div className="px-4 sm:px-8 py-16 max-w-5xl mx-auto">
            <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-6 crt-glow">
              // A lei como sistema operacional da sociedade
            </h2>
            <div className="border border-[color:var(--neon)]/60 p-4 sm:p-6 bg-black/80 backdrop-blur-md space-y-6">
              <p className="text-[color:var(--terminal)] text-sm sm:text-base leading-relaxed">
              ...
              <pre className="text-xs sm:text-sm bg-black/90 border border-[color:var(--neon)]/40 p-4 overflow-x-auto text-[color:var(--neon)] crt-glow">
...
> Sociedade = Hardware em evolução`}
          ...
          <div className="px-4 sm:px-8 py-16 max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {MODULES.map((m, i) => (
                <article
                  key={m.id}
                  className="border border-[color:var(--neon)]/70 p-5 bg-black/80 backdrop-blur-md hover:bg-(--neon)/10 transition-colors"
                >
        ...
        <section className="border-t border-[color:var(--neon)]/30 bg-transparent">
          <div className="px-4 sm:px-8 py-16 max-w-5xl mx-auto">
            ...
            <div className="border border-[color:var(--neon)]/70 overflow-x-auto bg-black/70 backdrop-blur-sm">
        ...
        {/* FOOTER */}
        <footer className="border-t border-[color:var(--neon)]/40 px-4 py-4 text-[10px] sm:text-xs text-[color:var(--terminal)] flex flex-wrap items-center justify-between gap-2 backdrop-blur-md bg-black/60 mt-auto">
